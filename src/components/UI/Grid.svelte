...
<div role="grid" class="mx-auto min-w-full">
  <div class={gridClass} style={gridSizeClass}>
    {#each grid as gridSegment (gridSegment.id)}
      <div
        class={` ${gridSegment.active ? "bg-purple-400 rounded-lg -m-1.5" : "rounded-lg -m-1.5 bg-transparent"} sm:p-4 flex justify-center items-center hover:bg-pink-500 transition-rounding transition-spacing duration-300`}
        on:click={(e) => {
          // console.log(e);
          // console.log(cols);
      ...
      >
        <div class="grid grid-rows-3 space-evenly grid-flow-col min-w-full">
          <div class="m-auto">
            <Dot color="transparent" isFilled={false} />
          </div>
          ...
          <div class="m-auto">
            <span
              class={`${textSize} ${gridSegment.active ? "text-white" : "text-black"}`}
