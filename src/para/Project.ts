...
import type { TaskConditionType } from '../types';
...
import { generateHeaderRegExp } from '../utils';
...
export class Project extends Item {
  ...
  async filter(
    ...
    while (true) {
      const momentDay = window.moment(day);
      const link = `${momentDay.year()}/Daily/${String(
        momentDay.month() + 1
      ).padStart(2, '0')}/${momentDay.format('YYYY-MM-DD')}.md`;
      const file = this.file.get(link, '', this.settings.periodic.periodicFolder);
  ...
  listByTime = async (
    ...
    const header = this.settings.periodic.daily.projectListHeader;
