...
import type { DateType, DateRangeType, BrainSettings } from '../types';
import { DAILY_REG, MONTHLY_REG, QUARTERLY_REG, WEEKLY_REG, YEARLY_REG } from '../constants';
...
export class Date {
  ...
  file: File;
  locale: string;
  constructor(app: App, settings: BrainSettings, file: File, locale: string) {
    this.app = app;
    this.file = file;
  ...
  lastDay(
    ...
    return {
      year: year ? window.moment().year(year).endOf('year').format('YYYY-MM-DD') : '',
      quarter:
        quarter && year
          ? window.moment()
            .year(year)
      ...
      week:
        week && year
          ? window.moment().year(year).week(week).endOf('week').format('YYYY-MM-DD')
          : '',
    };
  }
  ...
  days(
    ...
    if (week) {
      const from = year
        ? window.moment().year(year).week(week).startOf('week').format('YYYY-MM-DD')
        : null;
      const to = year
        ? window.moment().year(year).week(week).endOf('week').format('YYYY-MM-DD')
        : null;
    ...
    if (year) {
      const from = window.moment().year(year).startOf('year').format('YYYY-MM-DD');
      const to = window.moment().year(year).endOf('year').format('YYYY-MM-DD');
  ...
  files(
    ...
    const currentDate = window.moment(from).clone();

    while (currentDate.isBefore(window.moment(to))) {
      ...
      const weekFile = this.file.get(
        weekLink,
        '',
        this.settings.periodic.periodicFolder
      );
